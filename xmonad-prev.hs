...
myTabConfig = def { activeColor         = myFocusedBackground
                  ...
                  , fontName            = "xft:CozetteVector:pixelsize=20:antialias=true"
                  , decoHeight          = 30
                  }
...
myXPConfig = defaultXPConfig { font = "xft:UbuntuMono:size=12"
                             , height = 40
                             }
...
myLayoutHook = avoidStruts 
                ...
                $ zr ||| flex ||| tcm ||| flexWide ||| mc ||| tall ||| grid ||| bsp 
    where
        zr      = renamed [Replace "zr"]
                    $ boringWindows
...
myKeys =
    ...
    , ("M-S-p",     spawn "/home/rkb/bin/windowmenu")
    ...
    , ("M-S-b",     windowPrompt myXPConfig Bring allWindows)
    ...
    , ("M-h",       bindOn LD [("zr", windowGo L False), ("flex 3col", windowGo L False), ("grid", windowGo L False), ("", sendMessage Shrink)])
    , ("M-l",       bindOn LD [("zr", windowGo R False), ("flex 3col", windowGo R False), ("grid", windowGo R False), ("", sendMessage Expand)])
    , ("M-j",       bindOn LD [("grid", windowGo D False), ("", BW.focusDown)])
    , ("M-k",       bindOn LD [("grid", windowGo U False), ("", BW.focusUp)])
